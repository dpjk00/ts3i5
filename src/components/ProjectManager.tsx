...
import SubStoryManager from './SubStoryManager';
...
const ProjectManager: React.FC = () => {
  ...
  const handleSetActiveStory = (id: number) => {
    setActiveStory(id);
    console.log("Story activated", id)
      ...
      {activeProject === null ? (
        <>
          <div className="project-info">
            <input className="mt-4 text-white" type="text" name="name" placeholder="Project Name" value={newProject.name} onChange={handleInputChange} />
            <input className="mt-3 text-white" type="text" name="description" placeholder="Project Description" value={newProject.description} onChange={handleInputChange} />
            <button className="bg-crimson hover:bg-blue-700 text-white mt-3 font-bold py-2 px-4 rounded-full" onClick={handleAddProject}>Add Project</button>
          </div>
          {editingProject && (
            <div className="edit-field">
              ...
              <button className="bg-crimson hover:bg-blue-700 text-white mt-3 font-bold py-2 px-4 rounded-full" onClick={handleUpdateProject}>Update</button>
              <button className="bg-crimson hover:bg-blue-700 text-white mt-3 font-bold py-2 px-4 rounded-full" onClick={() => setEditingProject(null)}>Cancel</button>
            </div>
          )}
          <div className="grid grid-cols-3 w-6/12 gap-6 mt-10">
            {projects.map(project => (
              <div className="flex flex-col w-100 border-2 border-white p-5" key={project.id}>
                ...
                <button className="bg-crimson hover:bg-blue-700 text-white mt-1 font-bold py-2 px-4 rounded-full" onClick={() => handleEditProject(project)}>Update</button>
                <button className="bg-crimson hover:bg-blue-700 text-white mt-1 font-bold py-2 px-4 rounded-full" onClick={() => handleDeleteProject(project.id)}>Delete</button>
                <button className="bg-green hover:bg-blue-700 text-white mt-1 font-bold py-2 px-4 rounded-full" onClick={() => handleSetActiveProject(project.id)}>Set Active</button>
              </div>
            ))}
          </div>  
        </>
          ...
          {projects.filter(project => project.id === activeProject).map(project => (
            <>
              <div className='flex justify-center items-center flex-col mt-4' key={project.id}>
                ...
                <button className="bg-crimson mt-2 hover:bg-blue-700 text-white mt-1 font-bold py-2 px-4 rounded-full" onClick={handleShowAllProjects}>Show All Projects</button>
              </div>
              {newStory && (
                <div className="flex justify-center items-center flex-col">
                  <div className="flex justify-center items-center flex-col">
                    ...
                    <button className="bg-crimson mt-2 hover:bg-blue-700 text-white mt-1 font-bold py-2 px-4 rounded-full" onClick={handleAddStory}>Add Story</button>
                  </div>
                </div>
              )}
                      ...
                      {activeStory !== null && (
                        ...
                        <div>
                          <div className="flex flex-col justify-center items-center">
                            <h2 className="text-2xl mt-5">Zadanie dla <b className='text-crimson'>{story.name}</b></h2>
                          </div>
